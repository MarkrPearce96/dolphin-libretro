...
public:
  ...
  // Append a backend after Initialize() — used by libretro to register its
  // virtual input backend without touching the platform-specific Init() path.
  void AddBackend(std::unique_ptr<ciface::InputBackend> backend);
